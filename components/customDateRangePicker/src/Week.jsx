...
export default class Week extends Component {
	render() {
		...
		let dayRange = moment.range(startOfWeek, endOfWeek);
		for (let day of dayRange.by("days")) {
			const onPress = () => {
				if (isDateBlocked(day)) {
					...
					const start =
						focusedInput === "startDate" ? day : startDate;

					const end = focusedInput === "endDate" ? day : endDate;

					if (start && end) {
						let smallRange = moment.range(start, end);
						for (let dayPeriod of smallRange.by("days")) {
